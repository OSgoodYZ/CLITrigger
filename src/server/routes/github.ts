...
function getGitHubConfig(projectId: string): GitHubConfig | null {
  const project = getProjectById(projectId);
  if (!project || !project.github_enabled || !project.github_token || !project.github_owner || !project.github_repo) {
    return null;
  }
  return {
...
router.get('/:projectId/test', async (req: PidReq, res: Response) => {
  const config = getGitHubConfig(req.params.projectId);
  if (!config) {
